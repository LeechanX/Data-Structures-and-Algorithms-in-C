...
void heap_hold(struct Heap *heap,int pos){
	int min=heap->data[pos],minpos=pos;
	int left=2*pos+1,right=2*pos+2;
	if(left<heap->size && heap->data[left]<min){
		min=heap->data[left];
		minpos=left;
	}
	if(right<heap->size && heap->data[right]<min)
		minpos=right;
	if(minpos!=pos){
...
int deletemin_heap(struct Heap *heap){
	if(heap_empty(heap))
		return -1;
...
int getmin_heap(struct Heap *heap){
	if(heap_empty(heap))
		return -1;
	return heap->data[0];
}
...
int main(int argc, char const *argv[])
{
	...
	printf("%dth bigest number=%d\n",k,getmin_heap(heap));
	printf("top%d bigest numbers:\n",k );
	while(!heap_empty(heap))
		printf("%d ", deletemin_heap(heap));
	printf("\n");
	return 0;
}